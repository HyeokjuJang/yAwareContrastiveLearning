...
class GeneralizedSupervisedNTXenLoss(nn.Module):
    def __init__(self, kernel='rbf', temperature=0.1, return_logits=False, sigma=0.0):
        ...
        if self.kernel == 'rbf':
            ...
            assert hasattr(
                self.kernel, '__call__'), 'kernel must be a callable'
    ...
    def forward(self, z_i, z_j, labels=None):
        ...
        if labels != None:
            assert N == len(
                labels), "Unexpected labels length: %i" % len(labels)
        ...
        # dim [N, N] => the diag contains the correct pairs (i,j) (x transforms via T_i and T_j)
...
class NTXenLoss(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        sim_zjj = sim_zjj - self.INF * torch.eye(N, device=z_i.device)
        correct_pairs = torch.arange(N, device=z_i.device).long()
        loss_i = func.cross_entropy(
            torch.cat([sim_zij, sim_zii], dim=1), correct_pairs)
        loss_j = func.cross_entropy(
            torch.cat([sim_zij.T, sim_zjj], dim=1), correct_pairs)
